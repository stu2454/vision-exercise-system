...
class TestPythonManifest:
    """The browser runs our real Python, so it must fetch the right files.

    The list is generated by walking imports. If it goes stale the failure
    appears in a participant's browser rather than here, which is the wrong
    place to find it.
    """
    ...
    def test_the_manifest_is_current(self):
        ...
        result = subprocess.run(
            [sys.executable, "tools/generate_web_manifest.py", "--check"],
    ...
    def test_the_manifest_covers_the_whole_scoring_path(self):
        manifest = json.loads((WEB / "python-manifest.json").read_text())
        files = set(manifest["files"])
        for required in (
            "src/live_session.py",
            "src/exercises/sit_to_stand.py",
            "src/movement/filtering.py",
            "src/movement/features.py",
            "src/pose/quality.py",
            "src/version.py",
            "config/exercises/STS-001.yaml",
        ):
            assert required in files, f"{required} missing from the manifest"

    def test_every_listed_file_exists(self):
        root = Path(__file__).resolve().parents[2]
        manifest = json.loads((WEB / "python-manifest.json").read_text())
        for name in manifest["files"]:
            assert (root / name).exists(), f"{name} is listed but missing"

    def test_the_scoring_path_needs_no_compiled_dependency(self):
        # Pyodide can run pure Python straight off. numpy or OpenCV in this
        # path would mean the browser could not run the real engine, and the
        # alternative is a second implementation.
        import ast

        root = Path(__file__).resolve().parents[2]
        manifest = json.loads((WEB / "python-manifest.json").read_text())
        banned = {"numpy", "cv2", "mediapipe"}
        for name in manifest["files"]:
            ...
            for node in ast.walk(tree):
                ...
                assert not (set(names) & banned), (
                    f"{name} imports a compiled dependency; the browser "
